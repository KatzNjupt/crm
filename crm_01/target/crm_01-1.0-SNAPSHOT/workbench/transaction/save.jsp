...
<script type="text/javascript" src="jquery/bootstrap-datetimepicker-master/locale/bootstrap-datetimepicker.zh-CN.js"></script>
	...
	<script>
		...
		由于数据量小，并且是键值对的关系，不适合放在数据库中
		这种情况我们需要用到properties属性配置文件进行保存

		stage2Possibility.properties
		01资质审查=10
		02需求分析=20

		Stage2Possibility.properties这个文件表示的是阶段和可能性的关系
		这种对应关系在交易阶段中经常使用
		所有我们需要把它解析在服务器缓存中
		application.setAttribute("stage2Possibility.properties")
		...
		$(function () {
			...
			$("#create-stage").change(function () {
				...
				目标：填写可能性
					...
					阶段有了stage
					阶段和可能性之间的对应关系，pMap是java中的键值对关系，是java中的map对象

					首先将pMap转为js中的键值对关系，json
				...
				/*
					json.key的形式不能取value
					因为今天的stage是一个可变的变量，
					如果是这样的key，不能以传统的json.key的方式来取值
					那么我们要取值的方式为json[key]
				 */
	...
	<form action="workbench/transaction/save.do" id="tranForm" method="post" class="form-horizontal" role="form" style="position: relative; top: -30px;">
		...
		<div class="form-group">
			...
			<div class="col-sm-10" style="width: 300px;">
				...
				<input type="hidden" name="activityId" value="2b974274461b447c9a97edd89f67f65d">
			</div>
		</div>
		...
		<div class="form-group">
			...
			<div class="col-sm-10" style="width: 300px;">
				...
				<input type="hidden" name="contactsId" value="86ece54be9734648a4df22a445ba0a4a">
			</div>
		</div>
